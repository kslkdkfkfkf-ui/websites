...
3. (pre-Pie) Start the Java process with app_process ([see here](https://aadb shell sh /storage/emulated/0/Android/data/moe.shizuku.privileged.api/start.shndroid.googlesource.com/platform/frameworks/base/+/oreo-release/cmds/pm/pm))
4. (Pie+) Exeadb shell sh /storage/emulated/0/Android/data/moe.shizuku.privileged.api/start.shcute the native program `cmd` ([see here](https://android.googlesource.com/platform/frameworks/native/+/pie-release/cmds/cmd/))
